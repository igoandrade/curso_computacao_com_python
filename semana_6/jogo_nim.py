...
def jogada_do_computador(n, m):
    ia_pieces_removed = ia_choice(n, m)
    if ia_pieces_removed == 1:
        print("\nO computador tirou uma peça.")
    else:
        print(f"\nO computador tirou {ia_pieces_removed} peças.")
    n -= ia_pieces_removed
    if n == 1:
...
def jogada_do_usuario(n,m):
    user_pieces_removed = int(input("\nQuantas peças você vai tirar? "))
    while (user_pieces_removed > m or user_pieces_removed <= 0):
        print("Oops! Jogada inválida! Tente de novo.")
        user_pieces_removed = int(input("Quantas peças você vai tirar? "))
    if user_pieces_removed == 1:
        print("Você tirou uma peça.")
    else:
        print(f"Você tirou {user_pieces_removed} peças.")
    n -= user_pieces_removed
    if n == 1:
        print("Agora resta uma peça no tabuleiro.")
    else:
        print(f"Agora restam {n} peças no tabuleiro.")
...
def ia_choice(n, m):
    choice = m
    while (((n - choice) % (m+1)) != 0) and choice >= 1:
        choice -= 1
    if choice == 0:
        return n
    else:
...
def partida():
    # Retorna '0' se o computador ganhou ou retorna '1' se o usuário ganhou a partida
    n = int(input("Quantas peças: "))
    m = int(input("Limite de peças por jogada? "))
    ia_starts = not((n % (m+1)) == 0)
    if ia_starts:
    ...
    while (n > 0):
        if ia_starts:
            n, player = jogada_do_computador(n, m)
            if testa_ganhou(n, player):
                return player
            n, player = jogada_do_usuario(n,m)
            if testa_ganhou(n, player):
                return player
        else:
            n, player = jogada_do_usuario(n,m)
...
def campeonato():
    ...
    for i in range(3):
        ...
        if resultado > 0:
            ...
            ia_points += 1
    print("\n**** Final do campeonato! ****\n")
    print(f"Placar: Você {user_points} X {ia_points} Computador")
